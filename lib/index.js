...
module.exports = function(db) {
  ...
  db.indico.streamBy = function(props, options) {
    ...
    if(idxStreamOpts.start.length !== idxStreamOpts.end.length || idxStreamOpts.start.length !== props.length) {
      throw new Error("Query `start` or `end` do not match the specified index");
    }


    idxStreamOpts.start.push(null);
    idxStreamOpts.end.push(undefined);

    idxStreamOpts.start = idxManager.encode(idxStreamOpts.start, true, true);
    idxStreamOpts.end = idxManager.encode(idxStreamOpts.end, true, true);
